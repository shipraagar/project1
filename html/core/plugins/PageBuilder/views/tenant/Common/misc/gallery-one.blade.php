                        ...
                        <div class="tab-menu-global tabStyleThree text-center mb-20">
                            <ul>
                                @foreach($data['categories'] ?? [] as $cat)
                                   <li><a href="#" class="{{$loop->first ? 'active' : ''}}" data-rel="tab-{{$loop->iteration}}">{{$cat->getTranslation('title',default_lang())}}</a></li>
                                @endforeach
                            </ul>
                        </div>
                                        ...
                                        <div class="col-lg-12 col-md-6">
                                            <figure class="singleGallery-global areaHeight1">
                                                ...
                                                <figcaption class="caption">
                                                    <h2>
                                                        <a href="{{$img_url}}" title="{{$gallery->getTranslation('title',default_lang())}}" class="image-popup">
                                                            {{$splited[0] ?? ''}} <span>{{$splited[1] ?? ''}}</span>
                                                        </a>
                                                    </h2>
                                                    <p>{{$gallery->getTranslation('subtitle',default_lang())}}</p>
                                                </figcaption>
                                            </figure>
                                        </div>
                                    </div>
