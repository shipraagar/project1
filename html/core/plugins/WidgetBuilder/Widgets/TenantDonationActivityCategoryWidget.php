...
class TenantDonationActivityCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_categories = DonationActivityCategory::where('status',1)->orderBy('id', 'DESC')->take($category_items)->get();
        $category_markup = '';
        foreach ($blog_categories as $item){

            $title = $item->getTranslation('title',$user_selected_language);
            $url = route('tenant.frontend.donation.activities.category', ['id' => $item->id,'any' => $title]);
