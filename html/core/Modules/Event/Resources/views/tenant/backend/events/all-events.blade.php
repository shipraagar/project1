                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <x-admin.header-wrapper>
                            <x-slot name="left">
                                <h4 class="card-title mb-5">{{__('All Events')}}</h4>
                                <x-bulk-action permissions="donation-delete"/>
                            </x-slot>
                            <x-slot name="right" class="d-flex">
                                <form action="{{route('tenant.admin.event')}}" method="get">
                                ...
                                <p></p>
                                <a href="{{route('tenant.admin.donation.new')}}"
    ...
    <script>
        (function ($) {
            ...
            $(document).ready(function () {
                <x-bulk-action-js :url="route('tenant.admin.event.bulk.action')"/>
            })
        })(jQuery)
    </script>
    <x-media-upload.js/>
        ...
        $(function () {
            $(document).ready(function () {
                $('.table-wrap > table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('tenant.admin.event',['lang' => $default_lang]) }}",
                    columns: [
