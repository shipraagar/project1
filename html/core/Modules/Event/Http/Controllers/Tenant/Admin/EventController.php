            ...
            return DataTables::of($donation_logs)
                ...
                ->addColumn('action', function($row){
                    ...
                    if ($admin->can('event-payment-delete')){
                        $action .= General::deletePopover(route('tenant.admin.event.payment.log.delete',$row->id));
                    }
                    if ($admin->can('event-payment-edit')){
                        ...
                        if($row->status == 1){
                            $action .= General::invoiceBtn(route('tenant.admin.event.invoice.generate'),$row->id);
                        }
