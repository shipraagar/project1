...
@section('title') {{__('All Donations Activity Categories')}} @endsection
...
@section('content')
        ...
        <div class="card">
            <div class="card-body">
                <x-admin.header-wrapper>
                    <x-slot name="left">
                        <h4 class="card-title mb-5">{{__('All Donation Activity Categories')}}</h4>
                    <x-bulk-action permissions="donation-activity-category-delete"/>
                    </x-slot>
                    ...
                    <x-slot name="tr">
                        @foreach($all_categories as $data)
                            <tr>
                                ...
                                <td>
                                @can('donation-activity-category-edit')
                                    <a href="#"
                                       ...
                                       title="{{__('Edit')}}"
                                       data-id="{{$data->id}}"
                                       data-action="{{route('tenant.admin.donation.activity.category.update')}}"
                                    ...
                                    @endcan
                                    <x-delete-popover :permissions="'donation-activity-category-delete'" url="{{route('tenant.admin.donation.activity.category.delete', $data->id)}}"/>
                                </td>
                            </tr>
                        @endforeach
                    </x-slot>
    ...
    @can('donation-activity-category-create')
        <div class="modal fade" id="new_testimonial" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{route('tenant.admin.donation.activity.category')}}" method="post" enctype="multipart/form-data">
    ...
    @can('donation-activity-category-edit')
        <div class="modal fade" id="testimonial_item_edit_modal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">{{__('Edit Donation Item')}}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function($){
            ...
            <x-bulk-action-js :url="route( 'tenant.admin.donation.activity.category.bulk.action')"/>
