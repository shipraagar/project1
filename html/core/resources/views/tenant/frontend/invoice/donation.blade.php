...
<table class="table-footer" width="100%">
    ...
    <tr>
        <td valign="top">
            <div>
                <p class="singleItems"><strong>{{__('Title : ')}}</strong> {{optional($donation_details->donation)->getTranslation('title',get_user_lang())}}</p>
                ...
                <p class="singleItems"><strong>{{__('Payment Status : ')}}</strong>{{ \App\Enums\DonationPaymentStatusEnum::getText($donation_details->status) }}</p>
                <p class="singleItems"><strong>{{__('Transaction ID : ')}}</strong>{{$donation_details->transaction_id}}</p>
            </div>
        </td>

        <td align="right">
            <div class="borderStyle">
                <h6 class="singleItems"><strong>{{__('Donated Amount : ')}}</strong>{{amount_with_currency_symbol($donation_details->amount)}}</h6>
                <h2 class=" border-top" ><strong>{{__('Total Amount : ')}}</strong> {{amount_with_currency_symbol($donation_details->amount)}}</h2>
            </div>
        </td>
    </tr>
    </tbody>
