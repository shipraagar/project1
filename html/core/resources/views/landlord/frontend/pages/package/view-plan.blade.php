                    ...
                    <div class="row theme-row g-4 row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 row-cols-xl-4 mt-2">
                        @foreach($themes as $theme)
                            ...
                            <div class="col position-relative view_page_theme">
                                <div class="theme-wrapper {{ $final_condition }}" data-theme="{{$theme->slug}}" data-name="{{$theme->getTranslation('title', $user_lang)}}">
                                    {!! render_image_markup_by_attachment_id($theme->image) !!}
                                    <h4 class="text-center title">{{$theme->getTranslation('title', $user_lang)}}</h4>

                                    @if($theme->theme_code == $default_admin_set)
                        ...
                        <div class="price-body features-view-all">
                            <ul class="features">
                                ...
                                @if(!empty($order_details->product_create_permission))
                                    <li class="check"> {{ sprintf(__('Product Create %d'),$order_details->product_create_permission )}}</li>
                                @endif

                                @if(!empty($order_details->campaign_create_permission))
                                    <li class="check"> {{ sprintf(__('Campaign Create %d'),$order_details->campaign_create_permission )}}</li>
                                @endif
