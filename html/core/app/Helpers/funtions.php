...
function get_modified_title_tenant($title)
{
    if (str_contains($title, '{h}') && str_contains($title, '{/h}'))
    {
        ...
        return $final_title = '<h2 class="tittle">'.str_replace('{h}'.$highlighted_word.'{/h}', $highlighted_text, $title).'</h2>';
    } else {
        return $final_title = '<h2 class="tittle">'. $title .'</h2>';
    }
}
...
function get_modified_title_tenant_event($title)
{
    ...
    {
        $text = explode('{h}',$title);
        $highlighted_word_first = explode('{/h}', $text[1]);
        $highlighted_word = explode('{/h}', $text[1])[0];

        $combine_without_hightlighs = implode('',$text);
        $final_explode_others = explode('{/h}',$combine_without_hightlighs);
        $final_explode_others_two = explode(' ',$final_explode_others[0]);

        $title_without_hightlight_text = array_diff($final_explode_others_two,$highlighted_word_first);
        ...
        return  '<h2 class="tittle">
                             '.implode(' ',$title_without_hightlight_text).'
                        <span class="textFlip tittleBgOne wow ladeInRight" data-wow-delay="0.0s">
                            '.$highlighted_text.'
                         </span>
                    </h2>';
    } else {
...
function get_all_main_feature_create_permission($order_details){
    ...
    $output.=  '<li class="single"> '.$check_image.' '.sprintf(__('Page Create : %d'),$order_details->page_permission_feature).'</li>';
    endif;

      if(!empty($order_details->blog_permission_feature)):
          $output.=  '<li class="single"> '.$check_image.''.sprintf(__('Blog Create : %d'),$order_details->blog_permission_feature ).'</li>';
    endif;

     if(!empty($order_details->product_permission_feature)):
         $output.=  '<li class="single"> '.$check_image.''.sprintf(__('Product Create : %d'),$order_details->product_permission_feature ).'</li>';
      endif;

    if(!empty($order_details->service_permission_feature)):
        $output.=   '  <li class="single"> '.$check_image.''.sprintf(__('Service Create : %d'),$order_details->service_permission_feature ).'</li>';
    endif;

    if(!empty($order_details->donation_permission_feature)):
        $output.=  '  <li class="single"> '.$check_image.''.sprintf(__('Donation Create : %d'),$order_details->donation_permission_feature ).'</li>';
    endif;

    if(!empty($order_details->job_permission_feature)):
        $output.=   '  <li class="single"> '.$check_image.''.sprintf(__('Job Create : %d'),$order_details->job_permission_feature ).'</li>';
    endif;

    if(!empty($order_details->event_permission_feature)):
        $output.=  '  <li class="single"> '.$check_image.''.sprintf(__('Event Create : %d'),$order_details->event_permission_feature ).'</li>';
    endif;

    if(!empty($order_details->knowledgebase_permission_feature)):
        $output.= '  <li class="single"> '.$check_image.''.sprintf(__('Knowledgebase Create : %d'),$order_details->knowledgebase_permission_feature ).'</li>';
    endif;

    if(!empty($order_details->portfolio_permission_feature)):
        $output.= '  <li class="single"> '.$check_image.''.sprintf(__('Portfolio Create : %d'),$order_details->portfolio_permission_feature ).'</li>';
    endif;
...
function render_product_star_rating_markup_with_count($product_object): string
{
    ...
    if ($sum > 0) {
        ...
        $rating_markup = '<div class="ratings mt-2"></div>';
    }
